...
void TMainForm::Get_StressTestFlag(void)
{	
	bool flag = INIFileH.read_DLStessTestFlag( "MultiPortFlashDownloadProject.ini", Application->ExeName );

    if(flag)
    {
        m_bEnableStressTest = true;
    }
    else
    {
        m_bEnableStressTest = false;
    }
}

void TMainForm::Save_StressTestFlag(void)
{
	INIFileH.write_DLStessTestFlag("MultiPortFlashDownloadProject.ini", Application->ExeName, m_bEnableStressTest);
}
